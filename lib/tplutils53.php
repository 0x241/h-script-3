...
function sendMailToUser($mail, $section, $consts = array(), $lang = "", $fname = "e-mails")
{
    ...
    prepval($consts, 2);
    return sendMail($mail, textVarReplace($txt[(string) $section . ".topic"], $consts), nl2br(textVarReplace($hdr["_header"] . $txt[(string) $section . ".message"] . $ftr["_footer"], $consts)), $_cfg["Sys_NotifyMail"]);
}
