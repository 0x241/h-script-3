...
*/
global $_DF;
$_DF = [["% H:i", "* j, Y", "MDYHI", "m/d/y h:m", "m/d/y", "m" => ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], "f" => ["yesterday", "today", "tomorrow"]]];
...
function getLangDir($lang = "")
{
    global $_GS;
    $dir = "tpl/";
    foreach ([getlang($lang), $_GS["mode"], $_GS["theme"]] as $d) {
        if (!sEmpty($d)) {
            if (is_dir($dir . $d)) {
                $dir .= $d . "/";
            }
...
function showInfo($code = "Completed", $url = "*", $data = [])
{
    $url = fullURL($url);
    $_SESSION["_show_info"][$url] = [$code, $data];
    goToURL($url);
}
function showSplash($code = "Completed", $url = "*", $data = [], $templ = "splash", $tm = 0)
{
    $url = fullURL($url);
    $_SESSION["_show_info"][$url] = [$code, $data];
    if ($tm < 1) {
...
function showFormInfo($code = "Completed", $form = "", $data = [])
{
    $_SESSION["_show_info"][getFormName($form)] = [$code, $data];
    goToURL(fullURL());
}
...
function sendMailToUser($mail, $section, $consts = [], $lang = "", $fname = "e-mails")
{
    global $_GS;
    global $_cfg;
    ...
    prepval($consts, 2);
    return sendMail($mail, textVarReplace($txt[$section . ".topic"], $consts), textVarReplace($hdr["_header"] . $txt[$section . ".message"] . $ftr["_footer"], $consts), $_cfg["Sys_NotifyMail"]);
}
function sendMailToAdmin($section, $consts = [])
{
    global $_cfg;
